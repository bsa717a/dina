...
Regi teammates can talk to Piper from Slack. Preferred path: `/piper tasks`, `/piper add <title>`, `/piper done <number>` (same Regi board as the web UI). 4StudentLives stays on Telnyx RCS. Setup, slash-command URL, and smoke checks: [`docs/slack-regi.md`](docs/slack-regi.md).
